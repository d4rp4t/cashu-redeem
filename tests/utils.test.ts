import { Wallet, getDecodedToken, Proof, Mint } from "@cashu/cashu-ts";
...
describe("cashuUtils", () => {
	describe("Get satoshi value of single unit", () => {
		it("Correctly gets value from one sat token: ", async () => {
			const singleUnitToken = getDecodedToken(
				"cashuBo2FteBxodHRwczovL3N0YWJsZW51dC51bWludC5jYXNoYXVjc2F0YXSBomFpSABBRr30qa-rYXCBo2FhAWFzeEA4Yjk0N2VlMDlmMmNlMDcxNWYyYWEyNDk3NGE4YTQxZDJkMjZhNDg2Y2NmNDRmY2RhNjVmMDE3Y2FhYzQwM2IwYWNYIQNvu8aKC4tQVq-Bxjto5nS8rApH-aLf79VV7eeq7QW7pA",
			);
			const mint = singleUnitToken.mint;
			const wallet = new Wallet(mint, { unit: singleUnitToken.unit });
	...
	describe("Get satoshi value of token", () => {
		it("Correctly gets value from one sat token: ", async () => {
			...
			const cashumint = new Mint(token.mint);
			const wallet = new Wallet(cashumint, { unit: token.unit });
...
describe("changeUtils", () => {
	...
	describe("getChangeForMint", () => {
		beforeEach(() => {
			...
			localStorage.setItem(
				"mints",
				JSON.stringify(["https://8333.space:3338", "https://stablenut.umint.cash"]),
			);
		});
...
describe("lightningUtils", () => {
	describe("createInvoice", () => {
		...
		it("Should throw error if amount is invalid", async () => {
			await expect(
				createInvoice(-10, new LightningAddress("d4rp4t@lifpay.me")),
		...
		it("Should throw error if address is invalid", async () => {
			await expect(
				createInvoice(10, new LightningAddress("d4rp4t@lifpay.mesdasdasdad")),
			).to.be.rejectedWith(Error);
		});
		...
		it("Should throw error if LNURL is valid but amount is invalid", async () => {
			it("Should throw error if amount is invalid", async () => {
				const lnurl =
					"LNURL1DP68GURN8GHJ7MRFVECXZ7FWD4JJ7TNHV4KXCTTTDEHHWM30D3H82UNVWQHKGDRJWQ68G0MNDC75GDESXGMNX3PH94ZYVDJ9956NVS6P94PRWVEE956RWS6YXEP5ZD3J8QENWFNRVDUN642NGSS95XQ4";
				await expect(createInvoice(-10, await fetchLNURLData(lnurl))).to.be.rejectedWith(
					Error,
				);
			});
		}).timeout(10000);
