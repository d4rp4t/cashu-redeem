import { Mint, Proof, Token } from "@cashu/cashu-ts";
...
// remove spent proofs from localstorage
export function removeUsedProofs(usedProofs: Proof[]) {
	...
	const updatedProofs = localProofs.filter(
		(proof: Proof) => !usedSecrets.has(proof.secret),
	);
	...
	const changes = await Promise.all(
		localMints.map(async (mint) => {
			try {
				...
				const { keysets } = await new Mint(mint).getKeySets();
