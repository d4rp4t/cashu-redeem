...
async function createInvoice(
	amount: number,
	source: WebLNProvider | LightningAddress | LNURLResponse,
...
async function handleLNURL(lnurlData: LNURLResponse, amount: number): Promise<string> {
	if (amount < lnurlData.minSendable || amount > lnurlData.maxSendable) {
		throw new Error(
			`Amount must be between ${lnurlData.minSendable} and ${lnurlData.maxSendable} sats`,
		);
	}
