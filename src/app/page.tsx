...
import { getDecodedToken, MeltQuoteResponse, Token, Wallet } from "@cashu/cashu-ts";
...
export default function CashuRedemption() {
	...
	const [lightningInput, setLightningInput] = useState<
		LightningAddress | LNURLResponse
	>();
	...
	const [cashuWallet, setCashuWallet] = useState<Wallet>();
	...
	const validateInput = useCallback(async () => {
		try {
			...
			if (!mint) {
				console.log(token);
				throw new Error("Token does not contain mint");
			}

			const wallet = new Wallet(decodedToken.mint);
			await wallet.loadMint();

			setCashuWallet(wallet);
			toast.loading("Calculating amount", { id: "redeem" });
			const keysetFee = wallet.getFeesForProofs(decodedToken.proofs);
			const amount = (await getSatValue(wallet, decodedToken)) - keysetFee;
	...
	const meltToken = useCallback(async () => {
		...
		try {
			...
			const newQuote = await cashuWallet.createMeltQuoteBolt11(secondInvoice);
			const { keep, send } = await cashuWallet.send(
				newQuote.amount + newQuote.fee_reserve,
				token.proofs,
				{ includeFees: true },
			);
			if (!send || send.length == 0) {
				throw new Error("Failed to select proofs for redemption!");
			}

			toast.loading("Finalizing redemption", { id: "redeem" });
			const { change } = await cashuWallet.meltProofsBolt11(newQuote, send);
	...
	useEffect(() => {
		...
		async function handleMelt() {
			if (
				((!settings.previewMeltQuotes && meltQuote) ||
					(!!autopayParam && meltQuote && token)) &&
				isSubscribed
			) {
				try {
					await meltToken();
	...
	return (
		<div className="min-h-screen bg-gray-100 py-6 flex flex-col justify-center sm:py-12 relative overflow-hidden">
			...
			>
				<div className="absolute inset-0 bg-gradient-to-r from-purple-400 to-violet-500 shadow-lg transform -skew-y-6 sm:skew-y-0 sm:-rotate-6 sm:rounded-3xl"></div>
			...
			<footer className="mt-8 text-center text-sm text-gray-600 relative z-10">
				...
				<p>
					Learn more at{" "}
					<a href="https://cashu.space" className="text-violet-600 hover:underline">
						cashu.space
					</a>
				</p>
			</footer>
			...
			<div
				className={"text-md absolute bottom-5 right-5 font-medium flex flex-col gap-3"}
			>
				<button
					className={
						"w-[200px] px-4 py-2  text-white bg-violet-600 rounded-md hover:bg-violet-500 focus:outline-none focus:ring-2 focus:ring-violet-300 transition duration-150 ease-in-out transform hover:scale-105"
					}
					disabled={false}
					onClick={() => setShowEmojiDecoder(true)}
				>
					⚡ Decode Emoji 🥜
				</button>
				<button
					...
					disabled={false}
					onClick={() => setShowTotalChange(true)}
				>
					View Stored Change
				</button>
			</div>
