...
export function CashChangeDisplay({ onClose }: { onClose: () => void }) {
	...
	const totalsByCurrency = change.reduce(
		(acc, curr) => {
			if (!acc[curr.unit]) {
				acc[curr.unit] = 0;
			}
	...
	const changeByMint = change.reduce(
		(acc, curr) => {
			if (!acc[curr.mint]) {
				acc[curr.mint] = [];
			}
	...
	return (
		...
		>
			...
			>
				...
				{isLoading ? (
					<div className="text-center py-8 text-gray-500">Loading change data...</div>
				) : change.length === 0 ? (
					<div className="text-center py-8 text-gray-500">There{"'"}s no change!</div>
				) : (
					<>
